...
def defsum():
	for sum1 in range(len(idList)):
		sum01.append(sum(scoreList[sum1]))
print(sum01)
...
def defnum2():	#2번 입력시 출력할 함수
	...
	if user_new in deleteIDList:
		...
		idList.append(user_new)
		scoreList.append([user_newsc1,user_newsc2,user_newsc3])
	print('\n\n\n')
	print(user_dic)
	print(idList)
	print(scoreList)
...
def defnum3():	#3번 입력시 출력할 함수
	...
	for y in user_dic.keys():
		print(y, f'\t{user_dic[y][0]}\t{user_dic[y][1]}\t{user_dic[y][2]}\t{sum(user_dic[y])}\t{sum(user_dic[y])/3:<.2f}',end='\t')
		if (sum(user_dic[y])/3) >= 70:
			print('합격')
		else:
			print('불합격')
	print('\n\n\n')
...
def defnum4():	#4번 입력시 출력할 함수
	print('==<< 합격생 목록입니다 >>==',end='\n\n')
	for num4 in range(len(user_dic)):
		if sum01[num4]/3 >= 70:
			print(idList[num4], '합격')
	print(end='\n\n\n\n')
